  ...
  return (
    <>
      <div className="mt-10">
        ...
        <LoginInput
          ...
          onChange={() => {
            ...
            setState(newState);
            setVerify(
              inputRef.current.value === inputRefVerify.current.value &&
      ...
      <ul className="flex flex-col gap-1 mt-3">
        {["حداقل ۸ کاراکتر", "شامل حروف انگلیسی", "شامل عدد"].map(
          (list, index) => (
            <li
              className={`flex items-center gap-1 ${
                state[index] ? "text-success" : "text-default-500"
              }`}
              key={index + 1}
            >
              <div className="size-[18px] flex justify-center items-center">
                <span
                  className={`text-lg inline-block ${
                    state[index]
                      ? "icon-light-linear-Tick"
                      : "size-[6px] rounded-full bg-default-400"
                  }`}
                ></span>
              </div>
              <p>{list}</p>
